...
static void handleConnectionLostEvent(
...
static void handleDeviceEvent(const LEAP_DEVICE_EVENT *device_event) {
	...
	if (result == eLeapRS_InsufficientBuffer) {
		...
		deviceProperties.serial = realloc(deviceProperties.serial,
				deviceProperties.serial_length);
		result = LeapGetDeviceInfo(deviceHandle, &deviceProperties);
		if (result != eLeapRS_Success) {
...
static void handleDeviceFailureEvent(
		const LEAP_DEVICE_FAILURE_EVENT *device_failure_event) {
	if (ConnectionCallbacks.on_device_failure) {
		ConnectionCallbacks.on_device_failure(device_failure_event->status,
				device_failure_event->hDevice);
	}
}
...
static void handleLogEvent(const LEAP_LOG_EVENT *log_event) {
	if (ConnectionCallbacks.on_log_message) {
		ConnectionCallbacks.on_log_message(log_event->severity,
...
static void handlePointMappingChangeEvent(
...
void deepCopyTrackingEvent(LEAP_TRACKING_EVENT *dst,
		const LEAP_TRACKING_EVENT *src) {
